...
- Once the Blockchain Platform is deployed on the Kubernetes cluster (which can
  ...
  blockchain network by clicking on _Launch the IBM Blockchain Platform_.
...
- #### Create your Vaccination Organization CA
  ...
  - Give it a <b>CA display name</b> of `Vaccination CA`, a <b>CA administrator enroll ID</b> of `admin` and a <b>CA administrator enroll secret</b> of `adminpw`, then click <b>Next</b>.
...
- #### Associate the voter organization CA admin identity
  - In the Nodes tab, select the <b>Vaccination CA</b> once it is running (indicated by the green box in the tile).
  - Click <b>Associate identity</b> on the CA overview panel.
  - On the side panel, select the <b>Enroll ID</b> tab.
  - Provide an <b>Enroll ID</b> of `admin` and an <b>Enroll secret</b> of `adminpw`. Use the default value of `Vaccination CA Admin` for the <b>Identity display name</b>.
  - Click <b>Associate identity</b> to add the identity into your wallet and associate the admin identity with the <b>Vaccination CA</b>.
...
- #### Use your CA to register identities
  - Select the <b>Vaccination CA</b> Certificate Authority and ensure the `admin` identity that was created for the CA is visible in the table.
  - The next step is to register an admin for the organization "Vaccination". Click on the <b>Register User +</b> button. Give an <b>Enroll ID</b> of `vaccinationAdmin` and an <b>Enroll secret</b> of `vaccinationAdminpw`. Set the <b>Type</b> for this identity as `admin`. Specify to <b>Use root affiliation</b>. Leave the <b>Maximum enrollments</b> field blank. Click <b>Next</b>.
...
- #### Create the peer organization MSP definition
  ...
  - Enter the <b>MSP display name</b> as `Vaccination MSP` and the <b>MSP ID</b> as `vaccinationmsp`. Click <b>Next</b>.
  - Specify `Vaccination CA` as the <b>Root Certificate Authority</b>. Click <b>Next</b>.
  - Select the <b>New identity</b> tab. Give the <b>Enroll ID</b> and <b>Enroll secret</b> for your organization admin, i.e. `vaccinationAdmin` and `vaccinationAdminpw` respectively. Then, give the <b>Identity name</b> as `Vaccination Admin`.
...
- #### Create a peer
  ...
  - Give the <b>Peer display name</b> as `Vaccination Peer` and click <b>Next</b>.
  - On the next screen, select `Vaccination CA` as the <b>Certificate Authority</b>. Then, give the <b>Peer enroll ID</b> and <b>Peer enroll secret</b> as `peer1` and `peer1pw` respectively. Select the <b>Organization MSP</b> as `Vaccination MSP`. Leave the <b>TLS CSR hostname</b> blank and select `1.4.7-0` in the drop-down for <b>Fabric version</b>. Click <b>Next</b>.
  - Provide `Vaccination Admin` as the <b>Peer administrator identity</b> and click <b>Next</b>.
...
- #### Add organization as Consortium Member on the orderer to transact
  ...
  - Select the <b>Existing MSP ID</b> tab. From the drop-down list, select `Vaccination MSP (vaccinationmsp)`, as this is the MSP that represents the peer's organization "Vaccination".
...
- #### Create the channel
  ...
  - Under <b>Organizations</b>, select `Vaccination MSP (vaccinationmsp)` from the drop-down list to add the organization "Vaccination" as a member of this channel. Click the <b>Add</b> button. Set the permissions for this member as <b>Operator</b>. Click <b>Next</b>.
  - Leave the <b>Policy</b> as the default value i.e. `1 out of 1`. Click <b>Next</b>.
  - Select the <b>Channel creator MSP</b> as `Vaccination MSP (vaccinationmsp)` and the <b>Identity</b> as `Vaccination Admin`. Click <b>Next</b>.
...
- #### Join your peer to the channel
  ...
  - In the side panel that opens, under <b>Choose from available peers</b>, select `Vaccination Peer`. Once the peer is selected, a check mark will be displayed next to it. Ensure that <b>Make anchor peer(s)</b> is marked as `Yes`. Click <b>Join channel</b>.
...
## Step 4. Deploy vaccinationContract Smart Contract on the network
...
- #### Install a smart contract
  ...
  - Browse to the location of the vaccinationContract smart contract package file, which is in the root of the repo we cloned - the file is called `vaccinationContract.cds`.
...
- #### Instantiate smart contract
  - Under <b>Installed smart contracts</b>, find the smart contract from the list (**Note: ours is called vaccinationContract**) installed on our peer and click <b>Instantiate</b> from the overflow menu on the right side of the row.
  - On the side panel that opens, select the channel, `mychannel` on which to instantiate the smart contract. Click <b>Next</b>.
  - Select `Vaccination MSP` as the organization member to be included in the endorsement policy. Click <b>Next</b>.
  - Skip the <b>Setup private data collection</b> step and simply click <b>Next</b>.
  - Give the <b>Function name</b> as `init` and leave the <b>Arguments</b> blank. <b>Note: init is the method in the vaccinationContract.js file that initiates the smart contracts on the peer.</b>
...
- #### Connect with sdk through connection profile
  - Navigate to the <b>Organizations</b> tab in the left navigation, and click on <b>Vaccination MSP</b>.
  - Click on <b>Download Connection Profile</b>.
  - In the side panel that opens up, select `Yes` as the response for <b>Include Vaccination CA for user registration and enrollment?</b>. Under <b> Select peers to include</b>, select `Vaccination Peer`. Then click <b>Download connection profile</b>. This will download the connection json which we will use to establish a connection between the Node.js web application and the Blockchain Network.
...
- #### Create an application admin
  - Navigate to the <b>Nodes</b> tab in the left navigation, and under <b>Certificate Authorities</b>, choose <b>Vaccination CA</b>.
...
- #### Update application connection
  ...
  - Update the [config.json](server/config.json) file with:
    ...
    - The orgMSP ID, which we provided as `vaccinationmsp`.
    - The caName, which can be found in your connection json file under "organizations" -> "vaccinationmsp" -> certificateAuthorities". This would be like an IP address and a port. You should also use the `https` version of the certificateAuthorities line.
...
First, we need to register as a vaccination, and create our digital identity with
...
our vaccinationId and our name and registrarId.
