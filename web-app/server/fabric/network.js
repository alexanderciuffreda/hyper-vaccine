...
exports.connectToNetwork = async function (userName) {
  ...
  try {
    ...
    const network = await gateway.getNetwork('mychannel'); //CHANNEL NAME
...
//CUSTOMIZE FOR PERSON
exports.registerPerson = async function (personID, firstName, lastName, dateOfBirth, type, email, phone) {
  ...
  if (!personID || !firstName || !lastName || !dateOfBirth|| !type || !email || !phone) {
    let response = {};
  ...
  try {
    ...
    const userExists = await wallet.exists(personID);
    if (userExists) {
      let response = {};
      console.log(`An identity for the user ${personID} already exists in the wallet`);
      response.error = `Error! An identity for the user ${personID} already exists in the wallet. Please enter
    ...
    let response = `Successfully registered person ${firstName} ${lastName}. Use personID ${personID} to login above.`;
    return response;
  } catch (error) {
    console.error(`Failed to register person + ${personID} + : ${error}`);
    let response = {};
    response.error = error;
    return response;
  }
